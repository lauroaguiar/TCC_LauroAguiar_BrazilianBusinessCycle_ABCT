---
title: "A Teoria Austríaca dos Ciclos EConômicos Econômicos (TACE) é Capaz de Explicar os Ciclos Brasileiros?"
author: "Lauro Aguiar"
date: "`r Sys.Date()`"
output:
  pdf_document:
...
fontsize: 12pt     # Tamanho da fonte padrão (conforme ABNT)
...
bibliography: referencias.bib  # Arquivo de referências
csl: abnt.csl  # Estilo ABNT para citações e referências
---
...
```{r}
# Criação do vetor com todas as bibliotecas
pkg <- c(
  "scales",
  "gridExtra",
  "grid",
...
# Função para instalar os pacotes que não estão instalados
...
Função para importar séries manualmente (usar quando RBCB não funcionar)
...
get_bc_series <- function(code, start_date = "2004-01-01", end_date = "2024-12-01") {
  ...
  # Filtrar pelos dados após a data de início
  filtered_data <- data[data$data >= start_date & data$data <= end_date, ]
  
  # Verificar se há dados suficientes
  if (nrow(filtered_data) == 0) {
    stop("Nenhum dado disponível para o período especificado (entre start_date e end_date).")
  }
...
*Definição:* é Diferença entre as taxas de juros de LP e CP
...
Swaps DI pré-fixada com prazo de 180 dias.

**Taxa de Juros de Longo Prazo:** 

IDkA - Índice IDkA indexado ao IPCA com duração de 10 anos. 

POUP - Taxa de poupança.
...
# 1) Ler a base bruta
caminho_IDKA10 <- "C:\\Users\\lauro\\OneDrive\\Aleatório\\TCC\\BD\\IDKA10Bruta.xlsx" #Mudar o caminho!!!
IDKA10Bruta <- read_excel(caminho_IDKA10, sheet = "Historico")


# Fui tolo, era só baixar os dados diretamente do site do tesouro, já que são apenas duas planilhas (2005 e 2004). Ao menos aprendi sobre esse pacote.
...
#Filtro do título com vencimento em Ago/24 
...
# Inserindo 2004
caminho_NTNB24_2004 <- "C:\\Users\\lauro\\OneDrive\\Aleatório\\TCC\\BD\\NTNB24_2004.xlsx"
...
IDKA10 <- IDKA10Bruta %>%
  dplyr::select(`Data de Referência`, `Variação Diária (%)`) %>%
  mutate(
    Data = as.Date(`Data de Referência`, format = "%Y-%m-%d"),
    Ano = year(Data),
    Mes = month(Data),
    Trimestre = quarter(Data)
  ) %>%
  filter(
    # Filtrar o período entre o primeiro trimestre de 2006 e o último trimestre de 2024
    (Ano > 2005 | (Ano == 2005 & Trimestre == 4)) &  
    (Ano < 2024 | (Ano == 2024 & Trimestre <= 4)),
    # Filtrar apenas os meses de março, junho, setembro e dezembro
    Mes %in% c(3, 6, 9, 12)
  ) %>%
  group_by(Ano, Trimestre) %>%
  summarise(
    Taxa_Trimestral = mean(`Variação Diária (%)`, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
...
```{r}
# Calcular a média trimestral de yield_bid para 2005
#    (usando a média de TODOS os dias do trimestre, para manter
#     consistência com o método usado no código original do IDKA10)
...
NTNB24 <- NTNB24B %>%
  ...
  group_by(Ano, Trimestre) %>%
  # Calcular a média do yield_bid sobre TODOS os dias do trimestre.
  # Nomear a coluna como 'Taxa_Trimestral' para facilitar a junção futura.
  summarise(
    Taxa_Trimestral = mean(yield_bid, na.rm = TRUE),
    .groups = "drop" # Desagrupar após sumarizar
  ) %>%
...
# Saldo da Carteira de Créditos Livres R$ (milhões)
CREDL <- get_series(20542, start_date = "2007-03-01", end_date = "2024-12-01") %>% 
  pull('20542') %>%
  ts(start = c(2007, 03), frequency = 12)

# Saldo da Carteira de Créditos Direcionados R$ (milhões)
...
# Bens de Consumo Duráveis (START DATE - 01/01/2002)
...
# Indústria de Transformação (START DATE - 01/01/2002)
INDTR <- get_series(28505, start_date = "2003-12-01", end_date = "2024-12-01") %>% 
  pull('28505') %>%
  ts(start = c(2003, 12), frequency = 12)

# Bens Intermediários (START DATE - 01/01/2002)
BINT <- get_series(28507, start_date = "2003-12-01", end_date = "2024-12-01") %>% 
  pull('28507') %>%
  ts(start = c(2003, 12), frequency = 12)

# Bens de Consumo Semi e Não Duráveis (START DATE - 01/01/2002)
BCSND <- get_bc_series(28510, start_date = "2003-12-01", end_date = "2024-12-01")

# Utilização da Capacidade Industrial Instalada (START DATE - 30/04/1970)
...
Baseado em Neira, Bagus e Aria (2013) são feitas as seguintes razões de produção, a fim de validar se a expansão monetária gera maior alocação  em bens em estágios 
iniciais de produção comparado a bens em estágios iniciais.

```{r}
# 2.2.1.1 Índices da Indústria

# Calcular as razões de produção
IND1 <- log(BCAP / BINT) 
IND2 <- log(BCAP / BCSND) 
IND3 <- log(BCAP / INDTR) 
...
# Uso da função 'lag' do pacote 'stats' para séries temporais
library(stats)

# Índices de Produção com a função lag do pacote stats

IIPG <- log(INDG / stats::lag(INDG, 1))  # Usar stats::lag para objetos de série temporal
...
```{r}
# Índice Nacional de Preços ao Consumidor – Amplo (IPCA) (START DATE - 02/01/1980)
IPCA <- get_series(433, start_date = "2004-01-01", end_date = "2024-12-01") %>%
  pull('433') %>%
  ts(start = c(2004, 01), frequency = 12)

# Índice Nacional de Preços ao Consumidor – Amplo (IPCA) (START DATE - 02/01/1980) | Usada nas crédito.
IPCACredit <- get_series(433, start_date = "2007-03-01", end_date = "2024-12-01") %>%
  pull('433') %>%
  ts(start = c(2007, 03), frequency = 12)

#IPCAT
# Converter a série mensal em trimestral acumulando os fatores de crescimento
...
# Índice Nacional de Preços ao Consumidor – Amplo (IPCA) – Itens Livres (START DATE - 31/01/1992)
...
# Baixar a série do Federal Funds Rate do FRED
getSymbols("FEDFUNDS", src = "FRED")

# Limitar a série ao período de jan/2004 a dez/2024
ffrb <- window(FEDFUNDS, start = "2004-01-01", end = "2024-12-31")

# Selecionar apenas os meses que correspondem ao fim de trimestre: março, junho, setembro e dezembro
meses <- as.numeric(format(zoo::index(ffrb), "%m"))
ffr_quarterly <- ffr_monthly[meses %in% c(3, 6, 9, 12)]

# Converter a série para objeto de time series (ts) com frequência trimestral
...
# Arredondar os valores para facilitar a visualização
ffr <- round(ffr, 2)

# Plotar a série trimestral
plot(ffr, main = "Federal Funds Rate (FFR) - Média Trimestral",
     ylab = "Taxa (%)", xlab = "Ano")
```
...
```{r}
# 1) Instale e carregue o pacote ipeadatar (se ainda não estiver instalado)
# install.packages("ipeadatar")
# library(ipeadatar)
# 
# # 2) Busque todas as séries que tenham “Poupança Bruta” no título
# meta_poupanca <- series_search("Poupança Bruta")
# head(meta_poupanca, 10)
# # → Na saída, você verá colunas como:
# #   series_id      title                     unit       ...
# # 1 “XXXXXX”   “Poupança Bruta – Saldo”     “R$ Milhões”
# # 2 “YYYYYY”   “Poupança Bruta – Variação %” “%”
# 
# # 3) Anote o series_id exato da série que você quer (por ex. “XXXXXX”)
# #    e depois chame get_series(), especificando o período completo que deseja:
# dados_poupanca_bruto <- get_series(
#   code  = "XXXXXX",       # ← substitua aqui pelo series_id correto
#   first = "2004-01-01",   # data inicial
#   last  = as.character(Sys.Date())  # última data disponível
# )
# 
# # 4) Veja um resumo rápido
...
*Cálculo da Taxa de Poupança em relação à Renda Nacional Disponível e Poupança Bruta*

```{r}
# ----------------------------------------------
# Renomear coluna de código do trimestre
# ----------------------------------------------

DadosPoupancaBruto <- DadosPoupancaBruto %>%
  rename(
    Trimestre_Cod = "Trimestre (Código)",  # Coluna com formato "200304"
    Variavel_Cod = "Variável (Código)",
    Trimestre_Text = "Trimestre"
  )

# Selecionar e manipular variáveis
...
# ----------------------------------------------
# Conversão Trimestre para Data 
# ----------------------------------------------

# Verificar valores únicos de Trimestre_Cod
unique(DadosPoupanca$Trimestre_Cod)

# Extrair ano e trimestre do código (ex: "200304" → ano = 2003, trimestre = 4)
...
DadosPoupanca <- DadosPoupanca %>%
  mutate(
    Data = case_when(
      ...
      TRUE ~ NA  # Caso o trimestre seja inválido, retorna NA
    )
  )
...
# ----------------------------------------------
# Calcular as Taxas de Poupança
# ----------------------------------------------
DadosPoupanca <- DadosPoupanca %>%
  mutate(
    Taxa_Poupanca_Renda = (PoupancaB / Renda)*100,
    Poupanca = PoupancaB / (1 + vIPCAT/100) # 1) Deflação com IPCAT (IPCA Amplo 433)
  )
...
```{r}
# ----------------------------------------------  
# Aplicar Filtro HP (Dados até 2024T3)
# ----------------------------------------------

# Criar série temporal (2004Q4 até 2024Q4)
TxPoupSerie <- ts(
...
hp_resultado <- mFilter::hpfilter(TxPoupSerie, freq = 1600) #1600 É FUNDAMENTADO NO ARTIGO DE Hodrick–Prescott.
#Há outros parâmetros (type e drift) na função "hpfilter", mas até então o que importa é isso.
...
# ----------------------------------------------
# Passo 5: Visualização | Tendência
# ----------------------------------------------
ggplot(iPoupHP, aes(x = Data)) +
  geom_line(aes(y = Taxa_Poupanca_Renda, color = "Original"), linewidth = 0.8) +
  geom_line(aes(y = Tendencia_HP, color = "Tendência HP"), linewidth = 1.2) +
  labs(
    title = "Taxa de Poupança com Filtro Hodrick-Prescott (2004Q1 - 2024Q1)",
    y = "Taxa de Poupança (%)",
    color = "Legenda"
  ) +
  scale_color_manual(values = c("Original" = "#002C5E", "Tendência HP" = "#46B1E1")) +
  theme_minimal()
...
# supondo que OVER e SWAP já são ts mensais iniciando em 2004‐01
# Se suas séries ts já têm índice correto, você pode fazer:
...
# Determinar quais são os meses finais de cada trimestre
...
# Transformar em frequência trimestral correta
...
#  – para a taxa de poupança, em decimal
...
# 5) junte tudo num só objeto, usando apenas o período em que todas se sobrepõem
all4 <- ts.union(
  idka   = idka_ts,
  poup   = poup_ts,
...
# 7) Filtrar all4 para incluir apenas os dados até Q4 de 2024
...
ts.plot(S1_IDKAOVER, S2_POUPOVER,
        ...
        gpars = list(lwd = 2))
legend("topleft",
       legend = c("IDkA10−Selic","TaxaPoup−Selic"),
...
Deflacionamento, rebasing de índice, transformação logarítimica e conversão trimestral 
...
# Função para acumular um índice de preços a partir das variações mensais
acumular_indice <- function(variacao_mensal_ts) {
  # Verifica se é um objeto ts
  if (!is.ts(variacao_mensal_ts)) {
    stop("A entrada deve ser um objeto ts (time series).")
  }
  
  # Converte a variação percentual para fator de multiplicação (ex: 0.76% -> 1.0076)
  fatores <- 1 + variacao_mensal_ts / 100
  
  # Calcula o índice acumulado usando cumprod (produto cumulativo)
  # Define a base 100 para o primeiro período
...
# --- Crie os índices acumulados PRIMEIRO ---
# Supondo que IPCA, IPCAL, IPCACredit sejam os objetos ts com as variações mensais
...
# Função aprimorada para deflacionar uma série monetária usando o índice de preços ACUMULADO apropriado
deflacionar_serie <- function(serie_nominal, nome_serie) {
  # Determinar o tamanho da série
  n <- length(serie_nominal)
  
  # Selecionar o índice de preços ACUMULADO apropriado com base no nome da série
  if (nome_serie %in% c("CREDL", "CREDD")) {
    indice_precos_acumulado <- IPCACredit_idx
    if (n != length(IPCACredit_idx)) { # Compara com o tamanho do índice acumulado
       warning(paste("Alerta: A série", nome_serie, "tem", n, "observações, mas o IPCACredit_idx tem", length(IPCACredit_idx), "observações."))
    }
  } else if (nome_serie == "CREDT") {
    indice_precos_acumulado <- IPCA_idx
     if (n != length(IPCA_idx)) {
       warning(paste("Alerta: A série", nome_serie, "tem", n, "observações, mas o IPCA_idx tem", length(IPCA_idx), "observações."))
    }
  } else if (nome_serie %in% c("M1", "M2", "DV")) {
    indice_precos_acumulado <- IPCAL_idx
     if (n != length(IPCAL_idx)) {
       warning(paste("Alerta: A série", nome_serie, "tem", n, "observações, mas o IPCAL_idx tem", length(IPCAL_idx), "observações."))
    }
  } else {
    stop(paste("Série", nome_serie, "não reconhecida. Use uma das seguintes: CREDL, CREDD, CREDT, M1, M2, DV"))
  }
  
  # Verificar se os tamanhos são compatíveis ANTES de tentar deflacionar
  if (length(indice_precos_acumulado) != n) {
    stop(paste("Erro fatal: O tamanho da série nominal", nome_serie, "(", n, ") não corresponde ao tamanho do índice de preços acumulado selecionado (", length(indice_precos_acumulado), "). Verifique os períodos das séries."))
  }
  
  # Deflacionar: dividir a série nominal pelo índice de preços acumulado
  # Normalizar o índice para que o valor no primeiro período seja 1 (ou 100 se preferir)
  # Dividir pelo índice / índice[1] é o mesmo que multiplicar por índice[1] / índice
...
# Função para converter série mensal para trimestral usando média ou último valor
# converter_para_trimestre <- function(serie, metodo = "ultimo") {
#   if (metodo == "media") {
#     # Método 1: Média dos três meses
#     serie_trim <- aggregate(serie, nfrequency = 4, FUN = mean)
#   } else if (metodo == "ultimo") {
#     # Método 2: Usar o último mês de cada trimestre (mar, jun, set, dez)
...
converter_para_trimestre <- function(serie, metodo = "ultimo") {
  if (metodo == "media") {
    ...
    t1 <- time(serie)[1]
    ano_inicial <- trunc(t1)  # Parte inteira é o ano
    mes_inicial <- round((t1 %% 1) * frequency(serie)) + 1  # Parte decimal convertida para mês
    trimestre_inicial <- ceiling(mes_inicial / 3)  # Converter mês para trimestre
...
# Função para fazer rebasing de uma série (definir um período específico como 100)
fazer_rebasing <- function(serie, ano_base = 2003, periodo_base = 12) {
  # Localizar o valor no período base
  if (frequency(serie) == 12) {
    ...
    # Série trimestral - usamos o 4º trimestre de 2003
    valor_base <- window(serie, start = c(ano_base, 4), 
                         end = c(ano_base, 4))[1]
  }
  
  # Normalizar a série para que o período base seja igual a 100
...
# 1. SÉRIES MONETÁRIAS (M1, M2, DV)
#====================================================

# Deflacionar, converter para trimestre usando média e aplicar log
M1_processado <- M1 %>%
...
# 2. SPREADS (já estão em formato trimestral)
#====================================================
...
# 3. SÉRIES DE CRÉDITO (CREDT, CREDL, CREDD)
#====================================================
...
# 4. SÉRIES DE PRODUTO (IBCBR, IBCBR_Agro, IBCBR_Serv, IBCBR_Ind)
#====================================================
# Obter valor de dez/2003 para rebasing
# Supondo que as séries começam em jan/2004, precisamos obter esse valor
# Alternativa: usar o primeiro valor da série e escalonar

# Converter para trimestre (último valor) e fazer rebasing
IBCBR_processado <- IBCBR %>%
  converter_para_trimestre(metodo = "ultimo") %>%
  fazer_rebasing(ano_base = 2003, periodo_base = 4)  # 4º trimestre de 2003
...
# Nota: Para os índices já calculados (IND1-IND7) não é necessário rebasing
# pois são logaritmos de razões
...
#Na H3 (Exp. Monetária ➡ Indústria/Produção) será utilizadas as razões do Produto e não as séries originais do IBCBR
...
# UCAP já está em formato trimestral
UCAP_processado <- UCAP

# 6. IPCA (sem tratamento necessário)
#====================================================
# IPCA permanece mensal
# IPCAT já está em frequência trimestral
# IPCAL permanece mensal

# 7. SAVINGS
#====================================================
# A série DadosPoupanca$Poupanca já está em formato trimestral e está deflacionada pelo IPCAT

# Criar um objeto time series para poupança (se ainda não for)
Poupanca_ts <- ts(DadosPoupanca$Poupanca, 
                 start = c(2004, 1), frequency = 4)  # Supondo que começa em 2004-Q1


# Série de Poupança original começando em 2004
Poup2004 <- ts(DadosPoupanca$Poupanca, 
               start = c(2004, 1), frequency = 4)

# Encontrar o índice para o início de 2007
inicio_2007 <- which(time(Poup2004) >= 2007)[1]

# Criar série de Poupança começando em 2007
Poup2007 <- Poup2004[inicio_2007:length(Poup2004)]


# Criar série de Poupança começando em 2007, mantendo as propriedades de série temporal
...
# Verificar algumas séries processadas
...
# Criar uma lista com todas as séries processadas para fácil acesso
series_test <- list(
  M1 = M1_processado,
  M2 = M2_processado,
  DV = DV_processado,
...
```{r}
# CÓDIGO DO SPREAD - VERSÃO CORRIGIDA COM TAXAS REAIS
# Usando as funções já existentes
...
#====================================================
# FUNÇÕES AUXILIARES (já existentes no código original)
#====================================================
...
# Função CORRIGIDA para converter série mensal para trimestral
converter_para_trimestre <- function(serie, metodo = "ultimo") {
  if (!is.ts(serie)) {
    stop("A entrada deve ser um objeto ts (time series).")
  }
  ...
  if (metodo == "media") {
    ...
    if (length(meses_finais) == 0) {
      stop("Não há observações nos meses finais de trimestre (3, 6, 9, 12).")
    }
    
    # Calcular início da série trimestral
    ...
    if (length(valores_trimestrais) == 0) {
      stop("Não há valores válidos para criar a série trimestral.")
    }
    ...
    stop("Método deve ser 'media' ou 'ultimo'.")
  }
...
#====================================================
# PREPARAÇÃO DOS ÍNDICES DE PREÇOS
#====================================================

# Criar os índices acumulados PRIMEIRO
# CORREÇÃO: Usar IPCAL para deflacionar taxas de juros conforme especificado
IPCAL_idx <- acumular_indice(IPCAL)
...
# Função CORRIGIDA para deflacionar taxas de juros usando IPCAL
deflacionar_taxa_juros <- function(taxa_nominal_ts) {
  ...
  if (length(indice_precos_acumulado) != n) {
    # Alinhar as séries pelo menor período
    ...
    warning(paste("Séries alinhadas para", min_length, "observações."))
  }
  
  # Converter taxa para decimal
  taxa_decimal <- taxa_nominal_ts / 100
  
  # Calcular inflação mensal a partir do índice acumulado
  inflacao_mensal <- c(NA, diff(indice_precos_acumulado) / indice_precos_acumulado[-length(indice_precos_acumulado)])
  inflacao_mensal[1] <- 0  # Primeira observação
  
  # Fórmula de Fisher: (1 + r_real) = (1 + r_nominal) / (1 + inflação)
...
if (!exists("all4")) {
  stop("Objeto 'all4' não encontrado. Certifique-se de que foi criado anteriormente.")
}

cat("Estrutura de all4:\n")
print(str(all4))
cat("Dimensões de all4:", dim(all4), "\n")
cat("Período de all4:", start(all4), "a", end(all4), "\n")

# Extrair as séries de all4 (assumindo que estão em formato decimal)
selic_series <- all4[, "selic"]
swap_series <- all4[, "swap"]

# Converter para percentual se necessário (as séries parecem estar em formato decimal)
selic_percentual <- selic_series * 100
swap_percentual <- swap_series * 100

# Criar objetos ts explícitos
...
cat("Comprimento da série SELIC:", length(selic_ts), "\n")
cat("Comprimento da série SWAP:", length(swap_ts), "\n")
...
if (length(selic_ts) > 0 && length(IPCAL_idx) > 0) {
  ...
  stop("Erro: Séries SELIC ou IPCAL estão vazias.")
}
...
if (length(swap_ts) > 0 && length(IPCAL_idx) > 0) {
  ...
  stop("Erro: Séries SWAP ou IPCAL estão vazias.")
}

# Converter para trimestral (se as séries forem mensais)
if (frequency(selic_real) == 12) {
  ...
  # Se já forem trimestrais, usar como estão
...
ts.plot(S1_IDKAOVER_REAL, S2_POUPOVER_REAL,
        ...
        gpars = list(lwd = 2))
legend("topleft",
       legend = c("IDkA10−Selic Real","TaxaPoup−Selic Real"),
       col = c("#002C5E","#46B1E1"), lwd = 2)
...
ts.plot(S3_IDKASWAP_REAL, S4_POUPSWAP_REAL,
        ...
        gpars = list(lwd = 2))
legend("topleft",
       legend = c("IDkA10−SWAP Real","TaxaPoup−SWAP Real"),
...
cat("=== VERIFICAÇÃO DAS CONVERSÕES ===\n")
cat("Média SELIC nominal:", round(mean(selic_ts, na.rm = TRUE), 2), "%\n")
cat("Média SELIC real:", round(mean(selic_real, na.rm = TRUE), 2), "%\n")
cat("Diferença (nominal - real):", round(mean(selic_ts, na.rm = TRUE) - mean(selic_real, na.rm = TRUE), 2), "p.p.\n")

cat("\n=== ESTATÍSTICAS DOS SPREADS REAIS ===\n")
cat("S1 (IDkA10-SELIC Real) - Média:", round(mean(S1_IDKAOVER_REAL, na.rm = TRUE), 4), 
    "Desvio:", round(sd(S1_IDKAOVER_REAL, na.rm = TRUE), 4), "\n")
cat("S2 (POUP-SELIC Real) - Média:", round(mean(S2_POUPOVER_REAL, na.rm = TRUE), 4), 
    "Desvio:", round(sd(S2_POUPOVER_REAL, na.rm = TRUE), 4), "\n")
cat("S3 (IDkA10-SWAP Real) - Média:", round(mean(S3_IDKASWAP_REAL, na.rm = TRUE), 4), 
    "Desvio:", round(sd(S3_IDKASWAP_REAL, na.rm = TRUE), 4), "\n")
cat("S4 (POUP-SWAP Real) - Média:", round(mean(S4_POUPSWAP_REAL, na.rm = TRUE), 4), 
    "Desvio:", round(sd(S4_POUPSWAP_REAL, na.rm = TRUE), 4), "\n")

cat("\n=== INFORMAÇÕES ADICIONAIS ===\n")
cat("Período final dos spreads:", end(S1_IDKAOVER_REAL), "\n")
cat("Número de observações:", length(S1_IDKAOVER_REAL), "\n")
```
...
```{r}
# mapeamento das séries para suas legendas
var_labels <- c(
  M1           = "M1 (log)",
  M2           = "M2 (log)",
  DV           = "DV (log)",
  S1_IDKAOVER  = "S1_IDKAOVER (log – p.p.)",
  S2_POUPOVER  = "S2_POUPOVER (log – p.p.)",
  S3_IDKASWAP  = "S3_IDKASWAP (log – p.p.)",
  S4_POUPSWAP  = "S4_POUPSWAP (log – p.p.)",
  CREDT        = "CREDT (R$)",
  CREDL        = "CREDL (R$)",
  CREDD        = "CREDD (R$)",
  IND1         = "IND1 (Índice, dez/2003=100)",
  IND2         = "IND2 (Índice, dez/2003=100)",
  IND3         = "IND3 (Índice, dez/2003=100)",
  IND4         = "IND4 (Índice, dez/2003=100)",
  IND5         = "IND5 (Índice, dez/2003=100)",
  IND6         = "IND6 (Índice, dez/2003=100)",
  IND7         = "IND7 (Índice, dez/2003=100)",
  IIPG         = "IIPG (Índice, dez/2003=100)",
  PIB          = "PIB (Índice, dez/2003=100)",
  PIB_ExAgro   = "PIB_ExAgro (Índice, dez/2003=100)",
  PIB_Agro     = "PIB_Agro (Índice, dez/2003=100)",
  PIB_Ind      = "PIB_Ind (Índice, dez/2003=100)",
  PIB_Serv     = "PIB_Serv (Índice, dez/2003=100)",
  UCAP         = "UCAP (Índice)",
  Poup2004     = "Poup2004 (R$)",
  Poup2007     = "Poup2007 (R$)",
  FFR          = "FFR (% a.a.)"
...
for (serie_nome in names(series_test)) {
  ...
  # escolhe o título baseado no vetor; se não achar, usa o próprio nome
  titulo <- var_labels[serie_nome]
  if (is.na(titulo)) titulo <- serie_nome
  
  plot(serie,
       main = titulo,
       ylab = "",    # aqui você pode trocar por algo genérico ou específico
       type = "l",
       col  = "blue",
       las  = 1)
...
Classificação para Teste de Estacionaridade:

Para testes de Raiz Unitária (ADF, Phillips-Perron - H0: há raiz unitária):

Opção 1: "Nenhum" (sem intercepto, sem tendência)

Quando usar: Se a série visualmente parece flutuar em torno de zero e não tem tendência. O modelo de teste é Δy_t = γ*y_{t-1} + lags(Δy) + ε_t.

Sob a alternativa (estacionariedade), a série é estacionária com média zero.


Opção 2: "Intercepto" (ou "constante", "drift")

Quando usar: Se a série visualmente parece flutuar em torno de uma média constante diferente de zero, mas sem uma tendência sistemática de longo prazo. O modelo de teste é Δy_t = α + γ*y_{t-1} + lags(Δy) + ε_t.

Sob a alternativa (estacionariedade), a série é estacionária com média α/(1-γ). Se há raiz unitária (γ=0), α representa o drift.


Opção 3: "Tendência" (ou "intercepto e tendência")

Quando usar: Se a série visualmente exibe uma tendência linear clara (subindo ou descendo consistentemente ao longo do tempo). O modelo de teste é Δy_t = α + β*t + γ*y_{t-1} + lags(Δy) + ε_t.

Sob a alternativa (estacionariedade), a série é estacionária em torno de uma tendência determinística (α + β*t).

- A: Parece ter tendência clara (crescente ou decrescente). [TENDÊNCIA DETERMINÍSTICA e INTERCEPTO]
 
- B: Sem tendência clara, mas parece flutuar em torno de uma média não-zero. [INTERCEPTO]
 
- C: Sem tendência clara, parece flutuar em torno de zero (ou a média é visualmente insignificante). #ESTACIONÁRIA: [NONE] 
 
- D: Complexa (possíveis quebras estruturais, volatilidade muito variável – estas podem precisar de mais atenção individual depois).
...
*Passo 1:* Testar cada uma das séries temporais e determinar a sua ordem de integração (d)

*Passo 2:* Definir a ordem máxima (dmax) de integração do sistema
...
#------------------  FORMATAÇÃO DE P-VALOR COM ASTERISCOS  ------------------#
...
format_pvalue <- function(p, invert = FALSE) {
  ...
  if (!invert) {
    ...
    if (is.na(p)) {
      ...
      star <- "**"
    } else if (p > 0.05) {  # MODIFICAÇÃO: p > 0.05 recebe "*"
      star <- "*"
    }
  }
...
run_pp_kpss_adf <- function(serie, classe,
  ...
  cat("\n================  RESULTADOS  =================\n")
  cat("Classificação da série:", classe, "\n\n")
  ...
  # PP
  cat("Phillips–Perron  (pp.test)   H0: unidade raiz\n")
  for (nm in names(series_list)) {
    pval <- tryCatch(
...
# Criando colunas de resumo (α = 0.05)
alpha <- 0.05
...
# Criando coluna de ordem de integração
df_ordem <- data.frame(
...
for (i in 1:nrow(df_adf)) {
  if (df_adf$Nivel[i] < alpha && df_pp$Nivel[i] < alpha && df_kpss$Nivel[i] > alpha) {
    ...
    df_ordem$Ordem_Integracao[i] <- "Não determinada"
  }
}
...
#------ função auxilia­r para uma série ------
ordem_integracao <- function(i){
  # "Tem raiz unitária?" – se ADF NÃO rejeita (p >= alpha) ou KPSS rejeita estacionariedade (p <= alpha)
  ...
  # Avalia raiz unitária
  if (root_lvl) {                     # Se tem raiz no nível
    if (root_D1) {                    # Se ainda tem raiz na 1ª diferença
      return("I(2)")                  # Tem duas raízes unitárias -> I(2)
    }
    return("I(1)")                    # Só no nível -> I(1)
  } else {
    return("I(0)")                    # Sem raiz no nível -> I(0)
  }
}
...
#------ trata eventuais indeterminações ------
if (anyNA(order_vec)){
  cat("\nLimitação: em", sum(is.na(order_vec)),
      "série(s) a ordem não pôde ser decidida.\n")
} else {
...
# Supondo que series_finais e stationary_test já estejam no seu workspace

res_stat   <- numeric() # Estatística do teste
res_cv10   <- numeric() # Valor crítico a 10%
res_cv5    <- numeric() # Valor crítico a 5%
res_cv1    <- numeric() # Valor crítico a 1%
res_bp     <- integer() # Breakpoint
series_out <- character() # Nome da série

for (serie_name in names(series_test)) {
  cat("========================================\n")
  cat("Série:", serie_name, "\n")
  
  x         <- series_test[[serie_name]]
  za_result <- ur.za(x, model = "both", lag = NULL)
  
  # 1) Extrair a estatística do teste
  test_statistic <- za_result@teststat
  
  # 2) Extrair os valores críticos
  critical_values <- za_result@cval
  
  # 3) Extrair o breakpoint
  bp <- za_result@bpoint
  
  # 4) Imprimir para conferência
...
# --- 1) Extrai as séries originais como vetores ts ---
DV_ts    <- series_test[["DV"]]
CREDL_ts <- series_test[["CREDL"]]

# --- 2) Cria o calendário trimestral para cada série ---
...
# --- 4) Extrai o breakpoint de cada série no ZA e converte para data ---
...
p_dv <- ggplot(DVDF, aes(x = Date, y = Value)) +
  ...
  labs(title = "Série DV (4Q2004–4Q2024)",
       x     = "Data",
       y     = "DV") +
  theme_minimal()
...
p_credl <- ggplot(CREDLDF, aes(x = Date, y = Value)) +
  ...
  geom_vline(xintercept = bp_date_credl,
             ...
             size       = 0.8) +
  scale_x_date(date_breaks = "2 years", date_labels = "%y") +
  labs(title = "Série CREDL (1Q2007–4Q2024)",
       x     = "Data",
       y     = "CREDL") +
  theme_minimal()

# --- 7) Exibe os dois gráficos ---
print(p_dv)
print(p_credl)
...
```{r}
# Criar função para gerar dummies sazonais
...
# Criar dummies sazonais para as séries
...
# FUNÇÃO CORRIGIDA: Definir qual dummy usar baseado na variável
obter_dummies_corrigido <- function(var_nome) {
  if (var_nome == "Poup2004") {
    ...
    # Spreads (S1-S4_processado) não levam dummy sazonal
    return(NULL)
  } else {
    return(NULL)
  }
...
# FUNÇÃO CORRIGIDA: Obter nome da dummy
...
# Função auxiliar para obter dummies apropriadas baseada no nome da variável
...
```{r}
# Atualizar a matriz de dados endógenos incluindo UCAP_processado
endo_vars <- cbind(
...
# Criar dataframe específico para séries que começam em 2007
endo_vars_2007 <- endo_vars[which(time(endo_vars) >= 2007)[1]:nrow(endo_vars), ]

# 1. Definição das listas de variáveis conforme as hipóteses - ATUALIZADA
...
# Converter ffrm para série temporal se necessário
if (!is.ts(ffrm)) {
...
# Lista de todos os modelos que serão estimados - ATUALIZADA com modelos UCAP
modelos <- c(
...
*Passo 3:* Selecionar o número ótimo de defasagens (p) do modelo VAR;

```{r}
# Carregar bibliotecas necessárias
library(vars)

# Função para calcular lags ótimos (mantida igual)
  ...
  if (!is.null(exogen)) {
    ...
    tryCatch({
      ...
      if (nrow(exogen_window) == nrow(dados_modelo)) {
        ...
        cat("Aviso: Incompatibilidade de tamanho entre dados e FFR. FFR não será usado.\n")
      }
    }, error = function(e) {
      cat("Aviso: Erro ao alinhar FFR. FFR não será usado.\n")
    })
  }
  ...
  if (!is.null(dummies_list)) {
    if (!is.ts(dummies_list)) {
      tryCatch({
        ...
        cat("Aviso: Não foi possível converter dummies_list para série temporal.\n")
        return(NULL)
      })
    }
    ...
    tryCatch({
      ...
      if (nrow(dummies_window) == nrow(dados_modelo)) {
        ...
        cat("Aviso: Incompatibilidade de tamanho entre dados e dummies. Dummies não serão usadas.\n")
      }
    }, error = function(e) {
      cat("Aviso: Erro ao alinhar dummies. Dummies não serão usadas.\n")
    })
  }

  # Unir nomes das variáveis exógenas
...
for (modelo in modelos) {
  ...
  if (hipotese == "H1") {
    # Processar nomes das variáveis H1
    ...
    # Lógica específica para H1 - CORREÇÃO AQUI
    if (var2_nome == "Poup2004") {
      ...
      # Converter endo_vars_2007 para série temporal
      dados_usar <- ts(endo_vars_2007, start = c(2007, 1), frequency = 4)
      exogen_usar <- NULL
      # Usar D_Poup2007 diretamente (já está no período correto)
      dummies_usar <- D_Poup2007
      nome_dummy_usar <- "D_Poup2007"
    } else {
      # Para outros casos H1, não usar dummies
      dados_usar <- endo_vars
      exogen_usar <- NULL
      dummies_usar <- NULL
      nome_dummy_usar <- NULL
    ...
    # H3: FFR para séries S1-S4 E para SPREAD, dummies apropriadas
    dados_usar <- endo_vars
    ...
    # CORREÇÃO: Usar função corrigida para obter as dummies
    dummies_var1 <- obter_dummies_corrigido(var1_nome)
    dummies_var2 <- obter_dummies_corrigido(var2_nome)
    nome_var1 <- obter_nome_dummy(var1_nome)
    nome_var2 <- obter_nome_dummy(var2_nome)
    
    # Modificando para combinar ambas as dummies, se disponíveis
...
for (h in unique(resultados_lagsD$Hipotese)) {
  ...
  cat("Lag médio AIC:", round(mean(subset_h$AIC, na.rm = TRUE), 2), "\n")
  cat("Lag médio BIC:", round(mean(subset_h$BIC, na.rm = TRUE), 2), "\n")
  cat("Lag médio HQ:", round(mean(subset_h$HQ, na.rm = TRUE), 2), "\n")
}
...
# VERIFICAÇÃO: Mostrar quais exógenas estão sendo usadas para DV_processado e Spread
cat("\n=== VERIFICAÇÃO: EXÓGENAS PARA DV_processado e Spread ===\n")
...
# VERIFICAÇÃO ESPECÍFICA: Todos os modelos com SPREAD devem ter FFR
cat("\n=== VERIFICAÇÃO ESPECÍFICA: MODELOS COM SPREAD ===\n")
...
if (spread_com_ffr < nrow(spread_models)) {
  cat("\n⚠️  ATENÇÃO: Alguns modelos com Spread não estão usando FFR!\n")
  spread_sem_ffr <- spread_models[!grepl("FFR", spread_models$Exogenas), ]
  print(spread_sem_ffr[, c("Modelo", "Exogenas")])
} else {
  cat("\n✅ OK: Todos os modelos com Spread estão usando FFR corretamente.\n")
}

# VERIFICAÇÃO ADICIONAL: Modelos H1 com Poup2007
cat("\n=== VERIFICAÇÃO: MODELOS H1 COM Poup2007 ===\n")
...
*Passo 4:* Construir um modelo VAR com as variáveis em nível, independentemente da ordem de integração das variáveis relacionadas

Neste passo cada modelo é estimado considerando o *lag ótimo* encontrado na etapa anterior
...
varpD <- list() # Estes são VAR(p*) em nível
# Lista para armazenar os p_stars (lags BIC) utilizados
lags_bic_utilizados <- list()

# Função para estimar e salvar os modelos VAR (VAR(p*) em nível) com dummies
estimar_var_nivel_com_p_star <- function(df, var1, var2, model_name, lag_p_star, exogen = NULL, dummies = NULL) {
  # Seleciona as duas variáveis do modelo
  X <- df[, c(var1, var2)]
  colnames(X) <- c(var1, var2)
  
  # Garantir que X seja série temporal
  if (!is.ts(X)) {
    X <- ts(X, start = start(df), frequency = frequency(df))
  }

  # Converte lag_p_star para inteiro, caso ainda não seja
  lag_p_star_final <- as.integer(lag_p_star)
  
  # Combinar variáveis exógenas (FFR + dummies sazonais)
  exogen_total <- NULL
  
  # Adicionar FFR se fornecido
  if (!is.null(exogen)) {
    # Converter para série temporal se necessário
    if (!is.ts(exogen)) {
    ...
    if (nrow(exogen_aligned) == nrow(X)) {
      ...
      # MODIFICAÇÃO: Garantir que o nome da coluna seja FFR
  ...
  if (!is.null(dummies)) {
    # Garantir que dummies seja série temporal
    if (!is.ts(dummies)) {
    ...
    tryCatch({
      ...
      if (nrow(dummies_aligned) == nrow(X)) {
        if (!is.null(exogen_total)) {
          # MODIFICAÇÃO: Manter os nomes separados ao combinar
...
# Função auxiliar para obter o lag p* (baseado no BIC) do dataframe.
get_p_star_bic_from_df <- function(model_name_query, lags_df) {
  ...
  if (length(lag_bic_val) == 0) {
    stop(paste("Modelo", model_name_query, "não encontrado em resultados_lagsD."))
  }
  if (length(lag_bic_val) > 1) {
    warning(paste("Múltiplas entradas para o modelo", model_name_query, "em resultados_lagsD. Usando a primeira."))
...
# Garantir que endo_vars_2007 seja série temporal
...
# MODIFICAÇÃO: Garantir que ffrm_ts tenha o nome correto
if (ncol(ffrm_ts) == 1 && colnames(ffrm_ts)[1] != "FFR") {
  colnames(ffrm_ts) <- "FFR"
}

# --- Estimação dos modelos VAR(p*) em nível com dummies, usando lags BIC de resultados_lagsD ---

## Hipótese 1: Crédito não é causado pela poupança
...
## Hipótese 2: Aumento do agregado monetário causa aumento do spread
# Com ffr como variável exógena e dummies apropriadas
for (m in moedas) {
  for (s in spreads) {
...
## Hipótese 3: Aumento do spread ou da moeda causa variações na indústria e no produto agregado
# (a) Spread × Indústria (com ffr e dummies)
...
# (b) Spread × PIBs (com ffr e dummies)
for (s_var in spreads) {
  for (pib_var in pibs) {
...
# (c) Moeda × Indústria (sem ffr, mas com dummies)
for (m_var in moedas) {
...
# (d) Moeda × PIBs (sem ffr, mas com dummies)
for (m_var in moedas) {
  for (pib_var in pibs) {
...
# (e) Spread × UCAP (com ffr e dummies)
for (s_var in spreads) {
...
# (f) Moeda × UCAP (sem ffr, mas com dummies)
for (m_var in moedas) {
...
cat("Total de modelos VAR(p*) em nível estimados:", length(varpD), "\n")
...
# Verificar se alguns modelos têm variáveis exógenas
cat("\n\nVerificação de variáveis exógenas nos modelos:\n")
for(name in names(varpD)) {
  mod <- varpD[[name]]
  if (!is.null(mod$exogen)) {
    cat(name, ": tem", ncol(mod$exogen), "variáveis exógenas\n")
    # MODIFICAÇÃO: Mostrar os nomes das variáveis exógenas
    cat("  Variáveis exógenas:", paste(colnames(mod$exogen), collapse = ", "), "\n")
  }
}
...
# Lista para armazenar todos os modelos VAR
varpdmaxD <- list() # Estes agora serão VAR(p* + dmax) em nível
# Lista para armazenar os p_stars (lags BIC) utilizados
lags_bic_utilizados <- list()

# Função para estimar e salvar os modelos VAR (agora VAR(p* + dmax) em nível) com dummies
estimar_var_nivel_com_p_star <- function(df, var1, var2, model_name, lag_p_star, exogen = NULL, dummies = NULL) {
  # Seleciona as duas variáveis do modelo
  X <- df[, c(var1, var2)]
  colnames(X) <- c(var1, var2)
  ...
  # Calcula o lag a ser usado na estimação: p* + dmax
  lag_para_estimacao <- lag_p_star_final + dmax # ALTERAÇÃO PRINCIPAL
  ...
  # Salva o lag BIC (p*) utilizado para este modelo (p* em si, não p* + dmax)
  lags_bic_utilizados[[model_name]] <<- lag_p_star_final
...
# Verificar quantos modelos foram estimados e os lags BIC (p*) utilizados
cat("Total de modelos VAR(p* + dmax) em nível estimados:", length(varpdmaxD), "\n")
cat("Nomes dos modelos estimados:\n")
print(names(varpdmaxD))
cat("\nLags BIC (p*) e lags de estimação (p* + dmax) utilizados para cada modelo:\n")
if (length(lags_bic_utilizados) > 0) {
  for(name in names(lags_bic_utilizados)){
      cat(name, ": Lag p* =", lags_bic_utilizados[[name]], ", Lag de estimação (p* + dmax) =", lags_bic_utilizados[[name]] + dmax, "\n")
  }
} else {
...
```{r}
# Teste de Ausência de Autocorrelação Serial para modelos VARX com dummies

# 0) Verificar se dmax está definido
if (!exists("dmax")) {
  dmax <- 2
}
...
if (exists("resultados_lagsD") && "Modelo" %in% colnames(resultados_lagsD) && "BIC" %in% colnames(resultados_lagsD)) {
  ...
  stop("O dataframe 'resultados_lagsD' não existe ou não contém as colunas 'Modelo' e 'BIC'.")
}

# 2) Teste de Ausência de Autocorrelação Serial
...
if (!exists("varpdmaxD") || !is.list(varpdmaxD) || length(varpdmaxD) == 0) {
  stop("A lista 'varpdmaxD' com os modelos VARX(p+dmax) com dummies não foi encontrada ou está vazia.")
}
...
for (nome_modelo in names(varpdmaxD)) {
  ...
  tryCatch({
    ...
    if (length(lag_original_para_modelo) == 0) {
      warning(paste("Modelo", nome_modelo, "não encontrado em maxiorderD. Lag_Original será NA."))
...
# Adiciona as colunas de verificação de significância
...
# Coluna combinada para decisão sobre autocorrelação
autocorr_pdmaxD$Nao_Rejeita_h0 <- ifelse(
  ...
  ifelse(autocorr_pdmaxD$PT_nao_rejeita_10 & autocorr_pdmaxD$BG_nao_rejeita_10, "10%", 
         ifelse(autocorr_pdmaxD$PT_nao_rejeita_5 | autocorr_pdmaxD$BG_nao_rejeita_5, "5%", "Autocorrelação!")),
  "Autocorrelação!"
)
...
cat("\n=== RESULTADOS DOS TESTES DE AUTOCORRELAÇÃO SERIAL (MODELOS COM DUMMIES) ===\n")
print(head(autocorr_pdmaxD, 10))

# Modelos com problemas de autocorrelação
cat("\n\nModelos com indicação de autocorrelação serial:\n")
modelos_com_autocorr <- autocorr_pdmaxD[autocorr_pdmaxD$Nao_Rejeita_h0 == "Autocorrelação!" | 
...
if (nrow(modelos_com_autocorr) > 0) {
  ...
  cat("Nenhum modelo apresentou problemas de autocorrelação serial.\n")
}
...
cat("\n\nContagem de modelos por nível de rejeição:\n")
table(autocorr_pdmaxD$Nao_Rejeita_h0)

# Verificar influência das dummies comparando com modelos sem dummies (se existirem)
if (exists("autocorr_pdmax")) {
  cat("\n\n=== COMPARAÇÃO COM MODELOS SEM DUMMIES ===\n")
  ...
  cat("\nModelos que mudaram de status após inclusão de dummies:\n")
...
```{r}
# Criar cópias das novas colunas se ainda não existirem
...
# Lista de modelos com autocorrelação
modelos_autocorr <- c(
...
# Aplicar a função a todos os modelos e atualizar o dataframe
for (modelo in modelos_autocorr) {
  ...
  if (length(linha) == 0) {
    cat("AVISO: Modelo não encontrado no dataframe autocorr_pdmaxD\n")
    next
  }
  ...
  if (modelo %in% names(varpdmaxD)) {
    ...
    # Realizar novos testes de autocorrelação
    tryCatch({
      ...
      # Atualizar colunas de não rejeição
      ...
      # Determinar APENAS Status_Final (NÃO modificar Nao_Rejeita_h0)
      if (is.na(p_pt) || is.na(p_bg)) {
        ...
        autocorr_pdmaxD$Status_Final[linha] <- "Autocorrelação"
      }
...
# Para os modelos que não foram reestimados, preencher Status_Final com base no Nao_Rejeita_h0 original
for (i in 1:nrow(autocorr_pdmaxD)) {
  if (is.na(autocorr_pdmaxD$Status_Final[i])) {
    ...
    } else if (grepl("Autocorrelação", autocorr_pdmaxD$Nao_Rejeita_h0[i])) {
      autocorr_pdmaxD$Status_Final[i] <- "Autocorrelação"
    }
  }
}
...
cat("Modelos com autocorrelação persistente:", sum(autocorr_pdmaxD$Status_Final == "Autocorrelação", na.rm = TRUE), "\n")
...
```{r}
# Carregue o pacote vars se ainda não estiver
library(vars)

# Identifica os modelos com Status_Final == "ErroNaN" em autocorr_pdmax
modelos_erro_nan <- autocorr_pdmaxD$Modelo[autocorr_pdmaxD$Status_Final == "Autocorrelação"]

# Para cada modelo problemático, plota ACF e PACF de cada série residual
for (nome_modelo in modelos_erro_nan) {
  ...
  if (is.null(mod)) {
    warning("Modelo ", nome_modelo, " não encontrado em varpdmax. Pulando.")
    next
  }
  
  # Extrai os resíduos (matriz: observações × variáveis)
  res <- residuals(mod)
  
  # Número de variáveis no VAR
  nvar <- ncol(res)
  var_names <- colnames(res)
  
  # Para cada variável, plota ACF e PACF lado a lado
  for (i in seq_len(nvar)) {
    serie_res <- res[, i]
    nome_var  <- var_names[i]
    
    # Ajusta o layout: 1 linha × 2 colunas
    par(mfrow = c(1, 2), 
        mar   = c(5, 4, 4, 2) + 0.1)  # Aumenta as margens para acomodar o título
    
    # Plot ACF
    acf(serie_res,
        lag.max = 20,
        main    = paste("ACF dos resíduos\n", nome_modelo, "-", nome_var),
        xlab    = "Lag")
    
    # Plot PACF
    pacf(serie_res,
         lag.max = 20,
         main    = paste("PACF dos resíduos\n", nome_modelo, "-", nome_var),
         xlab    = "Lag")
  }
}

# Restaura layout padrão (1×1)
par(mfrow = c(1, 1))

```

*Passo 6.3:* Removendo os modelos com autocorrelação serial

```{r}
# Identificar modelos com autocorrelação
modelos_autocorr <- autocorr_pdmaxD$Modelo[autocorr_pdmaxD$Status_Final == "Autocorrelação"]

# Criar a nova lista removendo os modelos com autocorrelação
varpdmax <- varpdmaxD[!names(varpdmaxD) %in% modelos_autocorr]

# Imprimir o número de modelos removidos
cat("Número de modelos removidos:", length(varpdmax), "\n")
...
library(vars)
library(urca)  # Para testes de raiz unitária, se necessário
...
# Função para extrair informações e aplicar teste de causalidade
testar_causalidade_granger <- function(modelo_var, nome_modelo) {
  
  # Extrai as variáveis endógenas
  variaveis <- colnames(modelo_var$y)
  var1 <- variaveis[1]
  var2 <- variaveis[2]
  
  # Extrai a hipótese (H1, H2 ou H3)
...
granger$Causalidade_5 <- ifelse(granger$p_value < 0.05, "CAUSA", "NÃO CAUSA")
granger$Causalidade_10 <- ifelse(granger$p_value < 0.10, "CAUSA", "NÃO CAUSA")
```
...
heter <- imap_dfr(varpdmax, ~ {
  ...
  # 1) extraindo "H1", "H2" ou "H3" do início do nome
  mutate(
    hipotese    = substr(modelo, 1, 2),
    # 2) suas colunas de interpretação a 5% e 10%
    Resultados5  = if_else(p_value > 0.05,  "5%",  "Heterocedasticidade"),
    Resultados10 = if_else(p_value > 0.10, "10%", "Heterocedasticidade")
  ) %>%
  # 3) para cada hipótese, rankeia do maior p-valor (1) ao menor
  group_by(hipotese) %>%
  mutate(
...
stability <- imap_dfr(varpdmax, function(var_model, model_name) {
  ...
  # para cada equação (cada variável) dentro de stab$stability
  imap_dfr(stab$stability, function(efp_obj, var_name) {
    ...
    tibble(
      ...
      decisao     = if_else(
                       p_value < 0.05,
                       "Rejeita H0 → instabilidade",
                       "Não rejeita H0 → estável"
                     )
    )
  })
})
...
78 | Modelos que não possuem heterocedasticidade
...
# A ideia é selecionar os elementos de varpdmax cujos nomes estão na lista de aprovados
...
# Função para interpretar a rejeição da H0
...
for(i in seq_along(varxj)) {
  ...
  # Estatísticas e valores críticos para r = 0
  # Ajustamos os índices para usar números em vez de strings
...
# Combina tudo num único data.frame
...
## ⚠️ Disclaimer IRF/FEVD

As seções abaixo de impulse-funciton e decomposição de variância não foram incluídos no TCC, devido uma falta de adaptação no código para o método de Granger via T-Y. As seções serão atualizadas em breve. 
...
# agora corrija também as exógenas:
...
# 1) Abra o dispositivo gráfico PNG, definindo dimensões e resolução
png(
  filename = "irf_M2_to_S2.png",   # nome do arquivo de saída
  width    = 20,                   # largura
  height   = 16,                   # altura
...
# 3) Construa o gráfico com eixos customizados
irf_plot <- ggplot(irfdf, aes(x = h)) +
  ...
  scale_y_continuous(
    ...
    # remove zeros à direita para não mostrar sempre 4 casas
    labels = function(x) sub("\\.?0+$", "", sprintf("%.4f", x)),
    expand = c(0, 0)
  ) +
  labs(
    title   = "IRF: M2_processado → S2_processado",
...
# 4) Salve em PNG de alta resolução
ggsave(
...
# Função para gerar um gráfico IRF individual
gerar_irf_plot <- function(modelo_nome, varxj_obj) {
  
  # Extrair nomes das variáveis do título
  ...
  # 2) Corrigir o call com verificações adequadas
  cl <- mod$call
  cl[["p"]] <- mod$p
  
  # Remover qualquer parâmetro de lag que existir
  ...
  # Corrigir exógenas - só modificar se o slot existir
  if("exogen" %in% names(cl)) {
    # O slot existe no call, vamos atualizá-lo
    if("exogen" %in% names(mod)) {
      cl[["exogen"]] <- mod$exogen
    } else if("datamat" %in% names(mod) && !is.null(mod$datamat$exogen)) {
      cl[["exogen"]] <- mod$datamat$exogen
    } else {
      # Se o slot existe mas não encontramos dados, remover
      cl[["exogen"]] <- NULL
    }
  }
  # Se o slot não existe, não fazemos nada (modelo sem exógenas)
  ...
  tryCatch({
    ...
    p <- ggplot(irfdf, aes(x = h)) +
      ...
      labs(
        title = paste0(impulse_var, " → ", response_var),
        x = "Horizonte",
        y = "Resposta"
      ) +
      theme_minimal() +
      theme(
        # ALTERAÇÃO: Aumentando tamanhos dos elementos do gráfico
    ...
    # Se houver erro, retornar um gráfico vazio com mensagem detalhada
    ggplot() + 
...
painel_p2_final <- painel_p2 + 
  plot_annotation(
    title = "Funções de Impulso-Resposta H3 (Parte 2/2)",
    subtitle = "Horizonte: 12 trimestres | 95% CI Bootstrap (200 runs)",
    theme = theme(
      plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
      plot.subtitle = element_text(size = 14, hjust = 0.5)
...
# Alternativa: Salvar todos os 30 em uma única imagem (mantendo 6x5)
painel_completo <- wrap_plots(lista_plots, ncol = 5, nrow = 6) +
  plot_annotation(
    title = "Funções de Impulso-Resposta H3 - Painel Completo",
...
# Garante que a coluna com os nomes das séries seja a primeira
...
```{r}
install.packages("writexl")   # só na primeira vez
library(writexl)
...
# 1. define as datas de início/fim da faixa quarternária
...
credxpoup <- ggplot(df_all, aes(x = date, y = value, color = series)) +
  ...
  # faixa vertical cinza clara (colocada antes das linhas, para ficar atrás)
  annotate(
    "rect",
    xmin   = band_start,
    xmax   = band_end,
  ...
  scale_color_manual(
    ...
    labels = c(
      "CREDT_processado" = "Crédito Total",
      "Poup2004"         = "Poupança",
      "CREDD_processado" = "Crédito Direcionado",
      "CREDL_processado" = "Crédito Livre"
    )
  ) +
  
  # eixo x trimestral → anos
  scale_x_date(
    date_breaks = "1 year",
    date_labels = "%y"
  ) +
...
# salva em PNG com 300 dpi, 20×12 cm
ggsave(
  filename = "grafico_credito_poupanca.png",
  plot     = credxpoup,               # ou não especifique para usar o último gráfico
  device   = "png",
  width    = 20,
  height   = 12,
  units    = "cm",
...
# 2) Supondo que series_test seja uma lista contendo duas séries temporais
#    chamadas series_test$S2 e series_test$M2 (ts, zoo ou xts)
#    Converta ambas para zoo (se já forem zoo/xts, nada muda):
s2_z <- as.zoo(series_test$S2)
m2_z <- as.zoo(series_test$M2)

# 3) Junte as duas séries num único objeto zoo
...
# 5) Calcule a variação anual de M2 em porcentagem
df <- df %>%
  arrange(Date) %>%
  mutate(
    M2_Variation = (M2 / lag(M2, 4) - 1) * 100
  ) %>%
  filter(!is.na(M2_Variation))  # remove os primeiros trimestres sem variação

# 6) Determine o fator para escalar S2 ao eixo primário
fator <- max(df$M2_Variation, na.rm = TRUE) /
         max(df$S2_POUPOVER,   na.rm = TRUE)

# 7) Construa o gráfico com eixo secundário para S2
p <- ggplot(df, aes(x = Date)) +
  ...
  labs(
    title    = "M2 (Variação Anual) e S2_POUPOVER",
    subtitle = "Período: 1Q2005 - 4Q2024",
    x        = "Período",
    color    = "",
...
```{r}
# Função para criar cada gráfico (ajustada para melhor visualização)
criar_grafico_h2 <- function(serie1_nome, serie2_nome, series_test) {
  ...
  # Calcular variação anual da primeira série
  df <- df %>%
    arrange(Date) %>%
    mutate(
  ...
  # CORREÇÃO: Verificar se os spreads estão em decimal e converter para percentual
  # Se os valores estão entre 0 e 1, multiplicar por 100
  ...
  p <- ggplot(df, aes(x = Date)) +
    ...
    scale_y_continuous(
      ...
      # ALTERAÇÃO SIMPLES: Apenas aumentar frequência dos breaks
      breaks = scales::pretty_breaks(n = 10),  # Mais breaks no eixo primário
      labels = percent_format(scale = 1, suffix = "%"),
      sec.axis = sec_axis(
        transform = ~ . / fator,
        name = paste0(serie2_nome, " (%)"),
        # Mais breaks no eixo secundário também
    ...
    theme_minimal() +
    theme(
      # ALTERAÇÃO 1: Aumentar tamanho dos eixos
      # Para alterar manualmente, modifique os valores de size abaixo:
      axis.title = element_text(size = 14),      # Era 10, agora 14 (títulos dos eixos)
      axis.text = element_text(size = 12),       # Era 9, agora 12 (números dos eixos)
      axis.text.x = element_text(angle = 45, hjust = 1, size = 12),  # Eixo horizontal
      
      # ALTERAÇÃO 2: Aumentar tamanho do título
      plot.title = element_text(size = 18, face = "bold"),  # Era 14, agora 18
      
      # ALTERAÇÃO 3: Aumentar tamanho da legenda
      legend.text = element_text(size = 15),     # Era 9, agora 12
      legend.title = element_text(size = 13),    # Adicionado título da legenda
...
# Opção 1: Usar patchwork (recomendado)
library(patchwork)

# Criar layout 3x2 com espaço vazio no canto inferior direito
combined_plot <- (p1 | p2) / (p3 | p4) / (p5 | plot_spacer()) +
  plot_annotation(
    title = "Análise H2: Agregados Monetários vs Indicadores de Poupança",
    subtitle = "Período: 1Q2005 - 4Q2024",
    caption = "Fonte: Elaboração própria",
    theme = theme(
...
```{r}
# Função para criar cada gráfico (ajustada para variação trimestral)
criar_grafico_h2 <- function(serie1_nome, serie2_nome, series_test) {
  ...
  # Calcular variação TRIMESTRAL da primeira série (mudança principal)
  df <- df %>%
    arrange(Date) %>%
    mutate(
...
combined_plot <- (p1 | p2) / (p3 | p4) / (p5 | plot_spacer()) +
  plot_annotation(
    title = "Análise H2: Agregados Monetários vs Indicadores de Poupança",
    subtitle = "Período: 1Q2005 - 4Q2024 (Variação Trimestral)",
    caption = "Fonte: Elaboração própria",
    theme = theme(
      plot.title = element_text(size = 18, face = "bold", hjust = 0.5),
      plot.subtitle = element_text(size = 14, hjust = 0.5),
...
criar_grafico_h2 <- function(serie1_nome, serie2_nome, series_test) {
  ...
  # Manter a primeira série em nível (sem variação)
  df <- df %>%
    arrange(Date) %>%
    mutate(
  ...
  # Criar nomes para legenda (série em nível)
  ...
  p <- ggplot(df, aes(x = Date)) +
    ...
    coord_cartesian(ylim = y_limits) +  # Força os limites do eixo Y
    labs(
      title = paste0("H2: ", serie1_nome, " vs ", serie2_nome),
      x = "",
      color = ""
...
combined_plot <- (p1 | p2) / (p3 | p4) / (p5 | plot_spacer()) +
  plot_annotation(
    title = "Análise H2: Agregados Monetários vs Indicadores de Poupança",
    subtitle = "Período: 1Q2005 - 4Q2024 (Agregados Monetários em ln)",
    caption = "Fonte: Elaboração própria",
    theme = theme(
      plot.title = element_text(size = 18, face = "bold", hjust = 0.5),
      plot.subtitle = element_text(size = 14, hjust = 0.5),
